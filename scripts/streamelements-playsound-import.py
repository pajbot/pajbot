...
def main():
    ...
    for se_playsound in se_playsounds:
        ...
        print(
            f"{bot.prefix}add playsound {name} {link} --volume {volume} --cooldown {cooldown}{'' if enabled else ' --disabled'}"
        )
    ...
    print(
        ...
        f"\"{bot.prefix}eval bot.eval_from_file(event, 'https://pastebin.com/raw/link')\""
    )
