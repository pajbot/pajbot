def up(cursor, bot):
    ...
    # banphrase_data
    cursor.execute(
        'ALTER TABLE banphrase_data ADD FOREIGN KEY (added_by) REFERENCES "user"(id) ON DELETE SET NULL'
    )
    cursor.execute(
        'ALTER TABLE banphrase_data ADD FOREIGN KEY (edited_by) REFERENCES "user"(id) ON DELETE SET NULL'
    )
    ...
    cursor.execute(
        'ALTER TABLE command_data ADD FOREIGN KEY (added_by) REFERENCES "user"(id) ON DELETE SET NULL'
    )
    # In case you run into issues where this foreign key cannot be added because a referenced user does not exist, you can run the following query:
    ...
    cursor.execute(
        'ALTER TABLE command_data ADD FOREIGN KEY (edited_by) REFERENCES "user"(id) ON DELETE SET NULL'
    )

    # pleblist_song
    cursor.execute(
        "ALTER TABLE pleblist_song ADD FOREIGN KEY (stream_id) REFERENCES stream(id) ON DELETE CASCADE"
    )
    cursor.execute(
    ...
    cursor.execute(
        'ALTER TABLE prediction_run_entry ADD FOREIGN KEY (user_id) REFERENCES "user"(id) ON DELETE CASCADE'
    )
    ...
    cursor.execute(
        'ALTER TABLE roulette ADD FOREIGN KEY (user_id) REFERENCES "user"(id) ON DELETE CASCADE'
    )

    # stream_chunk
    cursor.execute(
        "ALTER TABLE stream_chunk ADD FOREIGN KEY (stream_id) REFERENCES stream(id) ON DELETE CASCADE"
    )
