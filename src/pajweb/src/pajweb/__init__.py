...
from utils import extend_version_if_possible
...
print(f"Static path: {static_path}, templates path: {template_path}")
...
def init(config_path: str) -> None:
    ...
    import utils
    import pajweb.common
    import pajweb.routes
    ...
    from pajweb.models import errors
    from pajweb.utils import download_logo, download_sub_badge
    ...
    pajweb.routes.admin.init(app)
    pajweb.routes.api.init(app)
    pajweb.routes.base.init(app)

    # Make a CSRF exemption for the /api/v1/banphrases/test endpoint
    csrf.exempt("pajweb.routes.api.banphrases.banphrases_test")
    csrf.exempt("pajweb.routes.api.playsound.playsound_test")

    pajweb.common.filters.init(app)
    pajweb.common.assets.init(app)
    pajweb.common.menu.init(app)

    app.register_blueprint(pajweb.routes.clr.page)

    errors.init(app, config)
    pajweb.routes.clr.config = config
    ...
    def current_time():
        current_time = {}
        current_time["current_time"] = utils.now()
        return current_time
