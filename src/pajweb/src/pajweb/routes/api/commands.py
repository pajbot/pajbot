...
def init(bp: Blueprint) -> None:
    ...
    def commands() -> ResponseReturnValue:
        commands = pajweb.utils.get_cached_commands()
    ...
    def command_get(raw_command_id) -> ResponseReturnValue:
        ...
        if command_id:
            command = find(
                ...
                pajweb.utils.get_cached_commands(),
            )
        else:
            command = find(
    ...
    @pajweb.utils.requires_level(500)
    ...
    def command_update(command_id: int, **extra_args) -> ResponseReturnValue:
        try:
            ...
            data = CommandUpdateSchema().load(json_data)
            assert isinstance(data, CommandUpdate)
        except ValidationError as err:
            return {"error": f"Did not match schema: {json.dumps(err.messages)}"}, 400

        payload = utils.remove_none_values(CommandUpdateSchema().dump(data))
    ...
    def command_checkalias(**extra_args):
        ...
        try:
            ...
            assert isinstance(data, CheckAlias)
        except ValidationError as err:
            return {"error": f"Did not match schema: {json.dumps(err.messages)}"}, 400
