...
def init(bp: Blueprint) -> None:
    @bp.route("/playsound-test/<playsound_name>", methods=["PUT"])
    def playsound_test(playsound_name: str, **options) -> ResponseReturnValue:
        # Create playsound
        ...
        try:
            ...
            data = CreatePlaysoundSchema().load(json_data, many=False)
            assert isinstance(data, CreatePlaysound)
    ...
    def playsound_edit(playsound_name: str, **options) -> ResponseReturnValue:
        ...
        try:
            ...
            if not json_data:
                return {"error": "Missing json body"}, 400
            data = UpdatePlaysoundSchema().load(json_data)
            assert isinstance(data, UpdatePlaysound)
        except ValidationError as err:
            return {"error": f"Did not match schema: {json.dumps(err.messages)}"}, 400
