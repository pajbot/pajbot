...
class MassPingProtectionModule(BaseModule):
    ...
    def on_pubmsg(self, source, message):
        ...
        timeout_duration = self.settings['timeout_length_base'] + self.settings['extra_timeout_length_per_ping'] * pings_too_many
        ...
        if self.settings['whisper_offenders']:
            self.bot.whisper(source.username, ('You have been timed out for {} seconds because your '+
                'message mentioned too many users at once.').format(timeout_duration))
