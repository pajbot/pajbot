...
class PaidUntimeoutModule(BaseModule):
    ...
    def load_commands(self, bot, **options):
            ...
            self.commands[
                ...
                examples=[
                    CommandExample(
                        ...
                        chat=f"user>bot:{bot.prefix}{self.settings['untimeout_command_name']}\nbot>user: You have been unbanned.",
                        description="",
                    ).parse()
                ],
            )
        if self.settings["unban_enable"]:
            self.commands[
                self.settings["unban_command_name"].lower().replace(f"{bot.prefix}", "").replace(" ", "")
            ] = Command.raw_command(
                self.unban_source,
                ...
                examples=[
                    CommandExample(
                        ...
                        chat=f"user>bot:{bot.prefix}{self.settings['unban_command_name']}\nbot>user: You have been unbanned.",
                        description="",
                    ).parse()
                ],
            )
