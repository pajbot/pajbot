...
class PaidTimeoutModule(BaseModule):
    ...
    def base_paid_timeout(self, bot, source, message, _time, _cost):
        ...
        with DBManager.create_session_scope() as db_session:
            ...
            if victim.timeout_end is not None and victim.timeout_end > now:
                ...
                bot.whisper(
                    victim,
                    f"{source} just timed you out for {_time} seconds. /w {bot.nickname} {bot.prefix}$unbanme to unban yourself for points forsenMoney",
                )
    ...
    def load_commands(self, **options):
        ...
        if self.settings["second_command"]:
            self.commands[
                self.settings["command_name2"].lower().replace(f"{bot.prefix}", "").replace(" ", "")
            ] = Command.raw_command(
                self.paid_timeout2,
