...
class HighlightModule(BaseModule):
    ...
    SETTINGS = [
        ModuleSetting(
            key="allow_subs", label="Allow subs to use !add highlight", type="boolean", required=True, default=False
        )
    ]
        ...
        if "id" in options:
            ...
            if len(options) > 0:
                ...
                if res is True:
                    bot.whisper(
                        source.username,
                        "Successfully updated your highlight ({0})".format(", ".join([key for key in options])),
                    )
                else:
                    bot.whisper(
                        source.username,
                        "A highlight with this ID does not exist, or something went terribly wrong (if this response is super late). Contact pajlada",
                    )
            else:
            ...
            bot.whisper(
                source.username,
                "Missing --id for which highlight to edit. Did you mean to create a new highlight? In that case, use !add highlight instead!",
            )
                ...
                "highlight": Command.raw_command(
                    ...
                    examples=[
                        CommandExample(
                            ...
                            description="Creates a highlight with the description 1v5 Pentakill",
                        ).parse(),
                        CommandExample(
                            None,
                            ...
                            "bot>user:Successfully created your highlight",
                            description="Creates a highlight with the description 1v5 Pentakill and an offset of 60 seconds.",
                        ).parse(),
                    ],
                )
            },
                ...
                "highlight": Command.raw_command(
                    ...
                    examples=[
                        CommandExample(
                            ...
                            description="Changes the offset to 180 seconds for the highlight ID 12",
                        ).parse(),
                        CommandExample(
                            None,
                            ...
                            description="Changes the description to '1v5 Pentakill PogChamp VAC' for highlight ID 12.",
                        ).parse(),
                        CommandExample(
                            None,
                            ...
                            description="Changes the link for highlight ID 12 to http://www.twitch.tv/imaqtpie/v/27878606",
                        ).parse(),
                        CommandExample(
                            None,
                            ...
                            "bot>user:Successfully updated your highlight (override_link)",
                            description="Changes the link for highlight ID 12 back to the twitch VOD link.",
                        ).parse(),
                    ],
                )
            },
                ...
                "highlight": Command.raw_command(
                    ...
                    examples=[
                        CommandExample(
                            ...
                            description="Removes the highlight ID 2",
                        ).parse()
                    ],
                )
            },
