...
class PlaySoundTokenCommandModule(BaseModule):
    ...
    def load_commands(self, **options):
        ...
        html_valid_samples = ''.join(['<tr><td class="command-sample">!#playsound {0.command}</td><td><script>var snd{0.command} = new Audio("{0.href}");</script><button onclick="snd{0.command}.play();" type="button">Play</button></td><td><button onclick="snd{0.command}.pause();snd{0.command}.currentTime=0;">Stop</button></td></tr>'.format(sample) for sample in Samples.all_samples])
