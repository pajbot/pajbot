...
class GivePointsModule(BaseModule):
    ...
    def give_points(self, bot, source, message, **rest):
        ...
        try:
            ...
            bot.whisper(source, f"{e}. Usage: {self.prefix}{self.command_name} USERNAME POINTS")
            return False
    ...
    def load_commands(self, **options):
        self.command_name = self.settings["command_name"].lower().replace(f"{self.prefix}", "").replace(" ", "")
        self.commands[self.command_name] = Command.raw_command(
            ...
            examples=[
                CommandExample(
                    ...
                    chat=f"user:{self.prefix}{self.command_name} pajapaja 4444\n"
