...
USHER_API = (
    "http://usher.twitch.tv/api/channel/hls/{channel}.m3u8?player=twitchweb"
...
class StreamThumbnailWriter:
    ...
    def __init__(self, streamer, ids):
        ...
        m3u8_obj, _ = self.get_live_stream()
        self.write_thumbnails(m3u8_obj)
    ...
    def write_thumbnails(self, m3u8_obj):
        for p in m3u8_obj.playlists:
            ...
            if quality == "Source":
                ...
                ts_uri = "{uri.scheme}://{uri.netloc}{short_path}/{ts_path}".format(
                    uri=parsed_uri, short_path=short_path, ts_path=last_line
                )
