...
def init(page):
    ...
    def moderators(**options):
        with DBManager.create_session_scope() as db_session:
            ...
            userlists["Broadcaster"] = list(filter(lambda user: user.level >= 1500, moderator_users))
            userlists["Super Moderators"] = list(filter(lambda user: 1000 <= user.level < 2000, moderator_users))
