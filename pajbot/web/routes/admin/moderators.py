...
def init(page):
    ...
    def moderators(**options):
        with DBManager.create_session_scope() as db_session:
            ...
            userlists["Broadcaster"] = list(filter(lambda user: 1500 <= user.level < 1999, moderator_users))
            userlists["Super Moderators"] = list(filter(lambda user: 1000 <= user.level < 1500, moderator_users))
