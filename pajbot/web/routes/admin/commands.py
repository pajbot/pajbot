...
def init(page):
    ...
    @requires_level(500)
    def commands_create(self, bot, **options):
        ...
        if "aliases" not in request.form:
            abort(403)
        alias_str = request.form.get("aliases", "").replace(f"{bot.prefix}", "").lower()
        ...
        alias_str = alias_str.replace(" ", "").replace(f"{bot.prefix}", "").lower()
