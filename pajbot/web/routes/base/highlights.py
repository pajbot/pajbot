...
def init(app):
    ...
    def highlight_list_date(date):
        ...
        highlights = (
            session.query(StreamChunkHighlight)
            .options(joinedload("*"))
            .filter(cast(StreamChunkHighlight.created_at, Date) == parsed_date)
            .order_by(StreamChunkHighlight.created_at.desc())
            .all()
        )
    ...
    def highlight_id(date, highlight_id, highlight_title=None):
        with DBManager.create_session_scope() as db_session:
            highlight = (
                db_session.query(StreamChunkHighlight).options(joinedload("*")).filter_by(id=highlight_id).first()
            )
            ...
            return render_template("highlight.html", highlight=highlight, stream_chunk=stream_chunk, stream=stream)
    ...
    def highlights():
        ...
        try:
            return render_template(
                "highlights.html", highlights=highlights[:10], dates_with_highlights=set(dates_with_highlights)
            )
        finally:
            session.close()
