...
def init(api):
    if not has_streamtip():
        ...
        log.info(
            "Check out the install/config.example.ini and set up the client_id and client_secret under the [streamtip] section"
        )
        return
    ...
    streamtip = oauth.remote_app(
        ...
        request_token_params={"scope": ["donations.read", "donations.create"]},
