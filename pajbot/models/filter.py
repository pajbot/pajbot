...
class Filter(Base):
    ...
    def __init__(self, action, f, **options):
        self.id = None
        self.name = "Filter"
        self.type = "banphrase"
        ...
        self.enabled = True
        self.regex = None
        self.set(action=action, f=f, **options)
    ...
    def init_on_load(self):
        ...
        if self.extra_extra_args:
            try:
                ...
                log.exception(
                    "Unhandled exception caught while loading Filter extra arguments ({0})".format(
    ...
    def is_enabled(self):
        return (
            ...
            and (self.regex is not None or self.type != "regex")
        )
