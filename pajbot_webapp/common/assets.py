...
def init(app):
    ...
    base_css = Bundle(
        "css/base.css", filters="cssmin", output="gen/css/base.%(version)s.css"
    )
    ...
    datetime_js = js_bundle(
        "scripts/datetime.js", "gen/scripts/datetime.%(version)s.js"
    )
    ...
    # playsounds)
    clr_overlay_js = js_bundle(
        "scripts/clr.overlay.js", "gen/scripts/clr.overlay.%(version)s.js"
    )
    ...
    admin_create_row = js_bundle(
        "scripts/admin/create_row.js", "gen/scripts/admin/create_row.%(version)s.js"
    )
    admin_edit_command = js_bundle(
        "scripts/admin/edit_command.js", "gen/scripts/admin/edit_command.%(version)s.js"
    )
    ...
    autolinker = js_bundle(
        "scripts/autolinker.js", "gen/scripts/autolinker.%(version)s.js"
    )
    assets.register("autolinker", autolinker)

    # Commands
    # Available under: commands_js
    commands_js = js_bundle(
        "scripts/commands.js", "gen/scripts/commands.%(version)s.js"
    )
    assets.register("commands_js", commands_js)

    # Pagination script
    # Available under: paginate_js
    paginate_js = js_bundle(
        "scripts/paginate.js", "gen/scripts/paginate.%(version)s.js"
    )
    assets.register("paginate_js", paginate_js)

    # common controls for the playsound pages
    playsound_common_js = js_bundle(
        "scripts/playsound.common.js", "gen/scripts/playsound.common.%(version)s.js"
    )
